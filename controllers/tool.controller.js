...
exports.getAllToolsAvailableForAlumni = async function (req, res)  {

    let data = await toolModel.getAllToolsAvailableForAlumni(req.params.numeroEstudante);
    ...
    if (data.kind === "ok") {
        ...
        res.status(500).json({ message: "Erro a selecionar todos os tools disponiveis" })
    }
    else {
